...
@main_bp.route("/pythonezltxa1d/cal/<tableName>/<columnName>", methods=['GET'])
def pythonezltxa1d_cal(tableName, columnName):
    '''
    计算规则接口
    '''
...
@main_bp.route("/pythonezltxa1d/file/download", methods=['GET'])
def pythonezltxa1d_file_download():
    '''
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythonezltxa1d/file/upload", methods=['POST'])
def pythonezltxa1d_file_upload():
    '''
    '''
...
@main_bp.route("/pythonezltxa1d/follow/<tableName>/<columnName>/<level>/<parent>", methods=['GET'])
def pythonezltxa1d_follow_level(tableName, columnName, level,parent):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythonezltxa1d/follow/<tableName>/<columnName>", methods=['GET'])
def pythonezltxa1d_follow(tableName, columnName):
    '''
...
@main_bp.route("/pythonezltxa1d/location", methods=['GET'])
def pythonezltxa1d_location():
    '''
...
@main_bp.route("/pythonezltxa1d/matchFace", methods=['GET'])
def pythonezltxa1d_matchface():
    '''
    baidubce百度人脸识别
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythonezltxa1d/option/<tableName>/<columnName>", methods=['GET'])
def pythonezltxa1d_option(tableName, columnName):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythonezltxa1d/remind/<tableName>/<columnName>/<type>", methods=['GET'])  #
def pythonezltxa1d_remind(tableName, columnName,type):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythonezltxa1d/sh/<tableName>", methods=['POST'])
def pythonezltxa1d_sh(tableName):
    '''
    '''
    if request.method == 'POST':
        ...
        logging.warning("pythonezltxa1d_sh.res=====>{}".format(res))
        return jsonify(msg)

@main_bp.route("/pythonezltxa1d/upload/<path:filepath>", methods=['GET'])
def pythonezltxa1d_upload(filepath):
    '''
    上传接口
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythonezltxa1d/group/<tableName>/<columnName>", methods=['GET'])
...
@main_bp.route("/pythonezltxa1d/value/<tableName>/<xColumnName>/<yColumnName>", methods=['GET'])
...
# 数据备份
@main_bp.route("/pythonezltxa1d/mysqldump", methods=['GET'])
def schema_mysqldump():
    if request.method == 'GET':
        filename = "mysql.dmp"
        filepath = '{}/api/templates/upload'.format(os.getcwd())
        cmd = "/usr/bin/mysqldump -h127.0.0.1 -uroot -P3306 -p123456 pythonezltxa1d > " + filepath + "/" + filename
        os.system(cmd)
        return send_from_directory(filepath, filename, as_attachment=True)
